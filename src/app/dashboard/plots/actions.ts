...
export async function sellPlot(formData: FormData) {
    ...
    if (!plot || !template) return { error: 'Invalid plot or plan' }

    // 3. Calculate Amounts
    const totalPrice = Number(plot.total_price)
    const downPayment = (totalPrice * Number(template.down_payment_percentage)) / 100
    const remainingAmount = totalPrice - downPayment
    const monthlyAmount = remainingAmount / template.total_months
    ...
    for (let i = 1; i <= template.total_months; i++) {
